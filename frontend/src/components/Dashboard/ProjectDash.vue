<template>
	<div class="project-ctn project">
		<div class="section left-section project__left-panel">
			<div class="title-ctn project__title">
				<h2>{{ projectLocal.title }}</h2>
			</div>
			<div class="state-collab-ctn project__state">
				<div class="state-collab-ctn__head project__state-head">
					<span>Etat des collaborations</span>
					<i
						class="bi bi-plus-square add-btn"
						v-if="projectLocal.stateUser === 'Admin'"
						title="Ajouter un nouveau besoin de collaboration"
						@click="changeFormAndOpen(formAddCollab)"
					></i>
				</div>
				<div class="state-collab-ctn__main project__state-main">
					<Tile
		...
		<div class="section middle-section">
			<BasicCtn headTitle="Description">
				<div
					:class="[projectLocal.description ? '' : 'no-desc', 'description']"
				>
					{{
			...
			<BasicCtn headTitle="Journal de bord"> </BasicCtn>
			<BasicCtn headTitle="Catégories">
				<template v-slot:btnHead
					><i
						...
						@click="changeFormAndOpen(formAddTag)"
					></i
				></template>
				<div class="tag-ctn">
					<span v-for="tag in projectLocal.tags" :key="tag"
						>{{ getNameTag(tag) }}
					</span>
				</div>
			</BasicCtn>
		...
		<div class="section right-section">
			<BasicCtn headTitle="Etat du projet">
				<template v-slot:btnHead
					><input
						class="btn-head-stateproject"
						type="button"
						:value="
							project.stateUser === 'Admin'
								? 'Supprimer le projet'
				...
				<div class="stateproject-ctn">
					<div class="stateproject-ctn__firstline">
						<span>{{
							projectLocal.stateUser === "Admin"
								? "Vous êtes administrateur sur le projet."
								: "Vous êtes collaborateur sur le projet"
						}}</span>
						<input
							v-if="project.stateUser === 'Admin'"
							class="btn-end-project"
							type="button"
							value="Déclarer terminé"
						/>
					</div>
			...
			<BasicCtn headTitle="Liste des collaborateurs">
				<div class="collabs-ctn">
					<div
						v-for="(collab, index) in getAllCollabsNames"
						:key="index"
						class="collab-line"
					>
			...
			<BasicCtn headTitle="Liens utiles">
				<template v-slot:btnHead
					><i
						...
						class="bi bi-plus-square add-btn"
						title="Ajouter une nouvelle catégorie"
						@click="changeFormAndOpen(formAddLinks)"
					></i
				></template>
				<div class="links-ctn">
					<div
